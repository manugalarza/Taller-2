...
import java.util.Set; //toco usarlo
...
public class SandboxArreglos
{
    ...
    public void volverPositivos( )
    {
    	...
    	for (int i = 0; i < arregloEnteros.length; i++) {
    		nuevoArreglo[i] = Math.abs(arregloEnteros[i]);
    	}
    	arregloEnteros = nuevoArreglo;
    }

    /**
     * Modifica el arreglo de enteros para que todos los valores queden organizados de menor a mayor. [Comentario mio, use quicksort]
     */
    public void organizarEnteros( )
    {
    	Sorts.quickSort(arregloEnteros, 0, arregloEnteros.length - 1);
    }
    ...
    {
    	Sorts.selectionSort(arregloCadenas);
    }
    ...
    public HashMap<Integer, Integer> calcularHistograma( )
    {
    	...
    	for (int i = 0; i < arregloEnteros.length; i++) {
            ...
            if (histograma.containsKey(key)) {
                conteo = histograma.get(key);
            }
            
            histograma.put(key, conteo + 1);
            }
    ...
    public int contarEnterosRepetidos( )
    {
    	...
    	Set<Integer> llaves = histograma.keySet();
    	for (Integer llave : llaves) {
    		Integer valor = histograma.get(llave);
    		if (valor > 1) {
    			contador++;
    		}
    	}
    ...
    public boolean mismosEnteros( int[] otroArreglo )
    {
    	...
    	for (Integer llave : histogramaOriginal.keySet()) {
    		Integer valor = histogramaOriginal.get(llave);
    		Integer valor2= histogramaOtro.getOrDefault(llave, 0);
    		if (!valor.equals(valor2)) {
    			return false;
    		}
    	}
    	for (Integer llave : histogramaOtro.keySet()) {
            if (!histogramaOriginal.containsKey(llave)) {
                return false;
            }
        }
